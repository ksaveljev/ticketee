  ...
  context "standard users" do
    ...
    context "with permission to view the project" do
      ...
      it "cannot delete a ticket without permission" do
        ...
        response.should redirect_to(project)
        flash[:alert].should eql("You cannot delete tickets in this project.")
      end

      it "can create tickets, but not tag them" do
        Permission.create(:user => user, :object => project, :action => "create tickets")
        post :create, :ticket => {:title => "New ticket!", :description => "Brand spankin' new" }, :project_id => project.id, :tags => "these are tags"
